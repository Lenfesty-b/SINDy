...
def sessionLCA_sindy(trials, signal, model_data, seed):
    ...
    for model in range(trials):
        ...
        np.random.seed(model + 20000+(seed -2000))
...
trials = 500 
seeds = [trials * i for i in range(20)] 
signals= np.arange(0.000, 0.041, 0.001)
filename = f'lcaddm_timecourse_combined.pkl'
with open(filename, 'rb') as f:
    test = pickle.load(f)
new_test=np.empty(41, dtype=object)
for j in range(41):
    new_test[j]=([np.array(test[j][2][i]).T for i in range(10000)])
...
model_coefs=[fit_sindy_model(new_test[i]) for i in range(41)]
...
for seed in seeds: 
    ...
    filename = f'lcaddm_timecourse_seedmulti_{seed}.pkl' 
...
with open('lcaddm_timecourse_multi_combined.pkl', 'wb') as f: 
    pickle.dump(combined_data, f) 
print("All seed iterations processed and combined into lcaddm_timecourse_multi_combined.pkl")