...
with open('lcaddm_timecourse_combined.pkl', 'rb') as f:
...
for snr_index in range(len(original_data)):
    ...
    fitted_coefs =([np.array(original_snr[2][i]).T for i in range(10000)])    # Fitted time in the second position
...
with open('lcaddm_timecourse_multi_combined.pkl', 'rb') as f:
...
# Now load fitted data and extract necessary columns
with open('/mnt/scratch2/users/blenfesty/lcaddm_timecourse_multi2_combined.pkl', 'rb') as f:
    fitted_data = pickle.load(f)
...
for snr_index in range(len(fitted_data)):
    ...
    fitted_choice = fitted_snr[0]   # Fitted choice in the first position
    fitted_time = fitted_snr[1]    # Fitted time in the second position
    #fitted_coefs =([np.array(fitted_snr[2][i]).T for i in range(10000)]) 

    # Update combined data with fitted values
    combined_data[snr_index].append(fitted_choice)
    combined_data[snr_index].append(fitted_time)
    #combined_data[snr_index].append(fit_sindy_model(fitted_coefs))

# Clear fitted_data from memory
del fitted_data,fitted_choice,fitted_time,fitted_snr

# Now load fitted data and extract necessary columns
with open('lcaddm_timecourse_multi0_combined.pkl', 'rb') as f:
    fitted_data = pickle.load(f)
...
with open('lcaddm_combined_multi_beh_coefs.pkl', 'wb') as f:
